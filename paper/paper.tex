...
experimentally evaluates it against two baselines (no containerization; Docker
without monitoring) and against itself at increasing levels of observability
(Docker only, Docker+Prometheus, Docker+Prometheus+Grafana). Across six
...
instrumentation overhead inside the application itself. All raw per-run data,
orchestration scripts, and an append-only decision log are published
...
containerized-but-unmonitored deployment (Baseline B). Every run is logged;
every deviation from the original plan --- including two genuine tooling
limitations encountered mid-study --- is recorded in an append-only decision
log alongside its resolution, so the results in Section~\ref{sec:results} are
traceable back to exact commands and raw data rather than summary claims.
...
throughput, latency, detection time, recovery time --- $n{=}20$ per
...
The pipeline architecture diagram in the project repository shows the
overall data flow: a developer pushes to
the Git repository, which triggers a GitHub Actions workflow that runs the
test suite and, on success, builds and pushes a Docker image. The image is
then run under Docker Compose alongside a cAdvisor container-metrics
exporter, a Prometheus server that scrapes both the application's native
\texttt{/metrics} endpoint and cAdvisor, and a Grafana instance provisioned
with Prometheus as its data source.
...
than to inter-service scheduling or network confounds (see decision log,
2026-09-09). The application exposes Prometheus metrics natively via the
...
installed; a small standard-library Python script
(\texttt{scripts/loadgen.py}) issues concurrent HTTP requests via
\texttt{ThreadPoolExecutor} and records per-request latency, trading raw
throughput ceiling for zero external dependencies and full inspectability.

\section{Experimental Scenarios}
\label{sec:scenarios}

Six experiments were run, summarized in Table~\ref{tab:scenarios}. Each is
described in more detail in the corresponding results subsection.
Orchestration scripts (\texttt{scripts/exp1\_deploy.py} through
\texttt{scripts/exp5\_failure\_recovery.py}) bring the relevant Docker
Compose profile up, execute the scenario, record raw per-run CSV rows, and
tear the stack down before the next run, so runs are independent.
...
Four configurations stand in for the paper's Baseline A/B/proposed-system
design: \emph{manual} (a raw \texttt{uvicorn} process, no container ---
Baseline A), \emph{Docker} (a locally built and started container, no CI/CD,
no monitoring --- Baseline B), \emph{Docker+CI/CD} (a real GitHub Actions
\texttt{workflow\_dispatch} run --- test job then Docker build/push --- timed
end-to-end, then deployed), and \emph{full pipeline} (Docker Compose bringing
up the application alongside cAdvisor, Prometheus, and Grafana --- the
proposed system). For each run, whatever was previously running is torn
...
was a transient race in polling for the newly dispatched run's ID (see
decision log) rather than a pipeline defect, so it is reported as observed
...
across all 60 runs (180$\times$300 = 18{,}000 requests). Throughput falls
monotonically as monitoring is added (2737 $\to$ 2641 $\to$ 2449 req/s,
Docker $\to$ +Prometheus $\to$ +Grafana), and mean/p95 latency both rise
slightly, consistent with host-level contention for a fixed core budget.
Counter-intuitively, the application container's own mean CPU\% \emph{falls}
over the same progression (11.0\% $\to$ 3.8\% $\to$ 1.8\%): as cAdvisor,
...
\texttt{docker stats} CPU reading drops even as end-to-end throughput and
latency both degrade. This is a genuine, non-obvious finding for RQ3:
...
All 20 induced CPU-stress runs were detected by Prometheus, with mean
detection time 5.65s (std 3.68s, min 1.51s, max 18.58s) and mean peak
...
under heavy contention (see decision log).
...
Prometheus's \texttt{up\{job="app"\}} series, mean 1.15s (std 0.21s, min
0.82s, max 2.03s) --- close to the theoretical floor set by the 2s scrape
interval combined with 0.3s poll granularity and Docker's own signal-delivery
latency. Recovery (container restart to health-check success) was
consistently $\approx$2.0s (std 0.006s, i.e.\ effectively deterministic),
which is essentially Uvicorn's own process startup time inside the
container: recovery time here is dominated by application boot, not by any
property of the monitoring stack.
...
a tooling race condition in the orchestration script, not evidence about the
pipeline's own robustness. A larger $n$ for the CI/CD arm (constrained here
by real GitHub Actions minutes and wall-clock cost, Section~\ref{sec:limitations})
would be needed to bound this reliability claim more tightly.
...
limitation, not a result of the experimental design, and is fully documented
...
instrumentation overhead. All raw data, orchestration scripts, and the
project's decision log are published in the accompanying repository to make
every figure in this paper independently reproducible.

\section*{Acknowledgment}
The author thanks [advisor name] for guidance on this study's design.
