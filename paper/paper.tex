...
measurements and analysis scripts underlying these results are available
...
containerized-but-unmonitored deployment (Baseline B). Every run was logged
...
installed; a small standard-library Python load generator issues concurrent
HTTP requests via \texttt{ThreadPoolExecutor} and records per-request
latency, trading raw throughput ceiling for zero external dependencies and
full inspectability.
...
described in more detail in the corresponding results subsection. For every
run, the relevant Docker Compose profile was brought up, the scenario was
executed, raw per-run measurements were recorded, and the stack was torn
down before the next run, so runs are independent of one another.
...
instrumentation overhead. The full raw measurement set underlying every
table in this paper is retained and available from the author upon
reasonable request, to support independent verification.
